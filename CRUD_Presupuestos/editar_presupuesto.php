...
$sql = "SELECT p.*, v.patente, v.marca as veh_marca, v.modelo as veh_modelo, c.nombre as cli_nom, c.apellido as cli_ape, c.numeroDocumentoCliente 
        FROM presupuestos p 
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (empty($detalles)) {
        ...
        try {
            ...
            // Actualizar Cabecera
            $sqlUpdate = "UPDATE presupuestos SET validez_dias=?, IDCliente=?, IDVehiculo=?, estado=?, notas_adicionales=?, casual_nombre=?, casual_apellido=?, casual_telefono=?, casual_patente=?, casual_tipo_vehiculo=?, casual_marca=?, casual_modelo=? WHERE IDPresupuesto=?";
            $stmtUp = $conn->prepare($sqlUpdate);
            $stmtUp->execute([
                $validez_dias, $IDCliente, $IDVehiculo, $estado, $notas_adicionales, 
                $casual_nombre, $casual_apellido, $casual_telefono, $casual_patente, $casual_tipo_vehiculo, $casual_marca, $casual_modelo, 
                $idPresupuesto
            ]);
            
            // Borrar detalles antiguos
            $conn->query("DELETE FROM detalle_presupuestos WHERE IDPresupuesto = $idPresupuesto");
            
            // Insertar nuevos detalles
            $sqlInsDet = "INSERT INTO detalle_presupuestos (IDPresupuesto, IDServicio, descripcion_libre, precio_unitario) VALUES (?, ?, ?, ?)";
            $stmtIns = $conn->prepare($sqlInsDet);
            ...
            foreach ($detalles as $d) {
                ...
                $stmtIns->execute([$idPresupuesto, $IDServicio, $desc_libre, $precio]);
            }
...
<head>
    ...
    <style>
        ...
        h2 { border-bottom: 2px solid var(--color-primario); padding-bottom: 10px; margin-top: 0; display: flex; justify-content: space-between; align-items: center; }
        .status-select { font-size: 14px; padding: 5px 10px; border-radius: 4px; border: 1px solid #ccc; }
        ...
        body.tema-oscuro .form-control, body.tema-oscuro .status-select { background: #3f3f46; color: #fff; border-color: #52525b; }
...
<body class="<?php echo $temaActual === 'oscuro' ? 'tema-oscuro' : ''; ?>">
            ...
            <div class="wrapper-content">
                <form method="POST" id="formPresupuesto" onsubmit="return validarFormulario()">
                    <div style="display:flex; justify-content:space-between; align-items:center; border-bottom: 2px solid var(--color-primario); padding-bottom: 15px; margin-bottom: 20px;">
                        <h2 style="margin:0; color: var(--color-primario);">Editar Presupuesto #<?php echo str_pad($idPresupuesto, 8, '0', STR_PAD_LEFT); ?></h2>
                        <div>
                            <span style="font-size: 13px; color: #666; margin-right: 15px; background: #eee; padding: 5px 10px; border-radius: 4px;"><strong>Creado por:</strong> <?php echo htmlspecialchars($presupuesto['creado_por'] ?? 'Sistema'); ?></span>
                    ...
                    <div class="form-section">
                        ...
                        <div class="form-row">
                            <div class="form-group" style="flex: 0 0 200px;">
                                ...
                                <input type="number" name="validez_dias" class="form-control" value="<?php echo htmlspecialchars($presupuesto['validez_dias'] ?? 15); ?>" min="1" required>
                            </div>
                            <div class="form-group" style="flex: 0 0 200px;">
                                ...
                                ?>
                                <select name="estado" class="form-control" id="estado_select" style="border-radius: 20px; font-weight: bold; text-align: center; text-align-last: center; color: <?php echo $e_color; ?>; background-color: <?php echo $e_bg; ?>; border-color: <?php echo $e_color; ?>;" onchange="actualizarColorEstado(this)">
                                    <option value="Pendiente" style="background-color: #fff3cd; color: #856404;" <?php echo $currE=='Pendiente'?'selected':''; ?> data-color="#856404" data-bg="#fff3cd">Pendiente</option>
                                    <option value="Aprobado" style="background-color: #d4edda; color: #155724;" <?php echo $currE=='Aprobado'?'selected':''; ?> data-color="#155724" data-bg="#d4edda">Aprobado</option>
                                    <option value="Rechazado" style="background-color: #f8d7da; color: #721c24;" <?php echo $currE=='Rechazado'?'selected':''; ?> data-color="#721c24" data-bg="#f8d7da">Rechazado</option>
                                    <option value="Vencido" style="background-color: #fdebd0; color: #d35400;" <?php echo $currE=='Vencido'?'selected':''; ?> data-color="#d35400" data-bg="#fdebd0">Vencido</option>
                                </select>
                                <script>
                                function actualizarColorEstado(selectElem) {
                                    var option = selectElem.options[selectElem.selectedIndex];
                                    selectElem.style.color = option.getAttribute('data-color');
                                    selectElem.style.backgroundColor = option.getAttribute('data-bg');
                                    selectElem.style.borderColor = option.getAttribute('data-color');
                                }
                                </script>
                            </div>
                        </div>
                    ...
                    <div class="form-section">
                        ...
                        <div class="radio-group">
                            <label><input type="radio" name="tipo_registro" value="manual" <?php echo !$isRegistrado ? 'checked' : ''; ?> onchange="toggleRegistro()"> Cliente/Vehículo Ocasional</label>
                            <label><input type="radio" name="tipo_registro" value="registrado" <?php echo $isRegistrado ? 'checked' : ''; ?> onchange="toggleRegistro()"> Buscar Existente</label>
                        </div>
                        
                        <!-- Bloque Búsqueda de Existentes -->
                        <div id="box_registrado" style="display:<?php echo $isRegistrado ? 'block' : 'none'; ?>; margin-top:15px;">
                            <div class="form-row" id="div_buscador" style="display:<?php echo $isRegistrado ? 'none' : 'block'; ?>;">
                            ...
                            <table class="client-table" id="tabla_vehiculos" style="display:<?php echo $isRegistrado ? 'none' : 'table'; ?>;">
                                <thead>
                                    <tr>
                            ...
                            <div id="paginacion_vehiculos" class="pagination-container" style="display:<?php echo $isRegistrado ? 'none' : 'flex'; ?>;"></div>
                            
                            <input type="hidden" name="IDVehiculo" id="IDVehiculo" value="<?php echo $presupuesto['IDVehiculo'] ?? ''; ?>">
                            <input type="hidden" name="IDCliente" id="IDCliente" value="<?php echo $presupuesto['IDCliente'] ?? ''; ?>">
                            
                            <div id="vehiculo_seleccionado" style="margin-top:15px; padding:15px; background:rgba(0,0,0,0.05); border-radius:4px; display:<?php echo $isRegistrado ? 'block' : 'none'; ?>; border:1px solid var(--color-primario);">
                                <div style="display:flex; justify-content:space-between;">
                                    <div>
                                        <h4 style="margin:0 0 5px 0; color:var(--color-primario);">Selección Confirmada</h4>
                                        <strong>Vehículo:</strong> <span id="lblVeh"><?php echo htmlspecialchars($presupuesto['patente'].' - '.$presupuesto['veh_marca'].' '.$presupuesto['veh_modelo']); ?></span><br>
                                        <strong>Cliente:</strong> <span id="lblCli"><?php echo htmlspecialchars($presupuesto['cli_nom'].' '.$presupuesto['cli_ape'].' ('.$presupuesto['numeroDocumentoCliente'].')'); ?></span>
                                    </div>
                        ...
                        <div id="box_manual" style="display:<?php echo !$isRegistrado ? 'block' : 'none'; ?>;">
                            <h4 style="margin-bottom:10px; color:#555; border-bottom:1px solid #eee; padding-bottom:5px;">Datos del Cliente</h4>
                            <div class="form-row">
                                <div class="form-group">
                                    <label>Nombre</label>
                                    <input type="text" name="casual_nombre" class="form-control" placeholder="" value="<?php echo htmlspecialchars($presupuesto['casual_nombre'] ?? ''); ?>">
                                </div>
                                <div class="form-group">
                                    <label>Apellido</label>
                                    <input type="text" name="casual_apellido" class="form-control" placeholder="" value="<?php echo htmlspecialchars($presupuesto['casual_apellido'] ?? ''); ?>">
                                </div>
                                <div class="form-group">
                                    <label>Teléfono (Opcional)</label>
                                    <input type="text" name="casual_telefono" class="form-control" placeholder="" value="<?php echo htmlspecialchars($presupuesto['casual_telefono'] ?? ''); ?>">
                                </div>
                            </div>
                            ...
                            <div class="form-row">
                                <div class="form-group">
                                    <label>Patente</label>
                                    <input type="text" name="casual_patente" class="form-control" style="text-transform:uppercase;" placeholder="" value="<?php echo htmlspecialchars($presupuesto['casual_patente'] ?? ''); ?>">
                                </div>
                                <div class="form-group">
                                    <label>Tipo</label>
                                    ...
                                    <?php else: ?>
                                        <select name="casual_tipo_vehiculo" class="form-control">
                                            <option value="Auto" <?php echo ($presupuesto['casual_tipo_vehiculo']=='Auto')?'selected':''; ?>>Auto</option>
                                            <option value="Camioneta" <?php echo ($presupuesto['casual_tipo_vehiculo']=='Camioneta')?'selected':''; ?>>Camioneta</option>
                                        </select>
                                    <?php endif; ?>
                                </div>
                            </div>
                            <div class="form-row">
                                <div class="form-group">
                                    <label>Marca</label>
                                    <input type="text" name="casual_marca" class="form-control" placeholder="" value="<?php echo htmlspecialchars($presupuesto['casual_marca'] ?? ''); ?>">
                                </div>
                                <div class="form-group">
                                    <label>Modelo</label>
                                    <input type="text" name="casual_modelo" class="form-control" placeholder="" value="<?php echo htmlspecialchars($presupuesto['casual_modelo'] ?? ''); ?>">
                                </div>
                            </div>
                        </div>
                    </div>
                            ...
                            <div style="min-width: 0;">
                                ...
                                <div id="servicios_asignados_container" style="overflow-x: hidden;">
                                    <table class="table-servicios" id="tabla_asignados" style="margin-top:0; table-layout: fixed; width: 100%;">
                                        ...
                                        <tbody>
                                            <tr id="row_vacia" style="display: <?php echo empty($detallesGuardados) ? 'table-row' : 'none'; ?>;"><td colspan="3" style="text-align: center; color: #888; padding: 20px;">No ha seleccionado ningún servicio.</td></tr>
                                        </tbody>
                                    </table>
                                </div>
                                
                                <div class="total-box">
                                    Total: $<span id="lblTotal"><?php echo number_format($presupuesto['total'], 2, '.', ''); ?></span>
                                </div>
                            </div>
                        </div>
                    </div>
                    ...
                    <div class="form-section">
                        ...
                        <textarea name="notas_adicionales" class="form-control" rows="3" style="resize: none;"><?php echo htmlspecialchars($presupuesto['notas_adicionales'] ?? ''); ?></textarea>
                    </div>

                    <div class="form-actions" style="justify-content: center;">
                        <button type="submit" class="btn-save">Actualizar Presupuesto</button>
                    </div>
                </form>
            </div>
        </main>
    ...
    <script>
        ...
        const detallesGuardados = <?php echo json_encode($detallesGuardados); ?>;
        let rowIndex = 0;
                ...
                detallesGuardados.forEach(d => {
                    ...
                    if (d.IDServicio) {
                        tr.innerHTML = `
                            <td style="padding-right: 10px;">
                                <input type="hidden" name="detalles[${rowIndex}][tipo]" value="catalogo">
                                <input type="hidden" name="detalles[${rowIndex}][IDServicio]" value="${d.IDServicio}">
                                <strong style="color:var(--color-primario);">${d.nombreServicio}</strong>
                            </td>
                            <td style="padding-right: 10px;">
                                <input type="number" class="form-control input-costo" name="detalles[${rowIndex}][precio_unitario]" value="${parseFloat(d.precio_unitario)}" step="0.01" oninput="recalcularTotal()" style="width: 100%; box-sizing: border-box; text-align: right; background: #fff;">
                            </td>
                            <td style="padding-right: 10px;">
                                <input type="text" class="form-control" name="detalles[${rowIndex}][descripcion_libre]" value="${d.descripcion_libre || ''}" placeholder="Detalles adicionales del procedimiento..." style="font-size: 13px; padding: 8px; width: 100%; box-sizing: border-box; background: #fff;">
                            </td>
                            <td style="text-align:center;">
                                <button type="button" class="btn-remove" onclick="this.closest('tr').remove(); revisarFilaVacia(); recalcularTotal();" title="Quitar Servicio" style="border-radius: 50%; width: 24px; height: 24px; padding: 0; line-height: 1;">✖</button>
                            </td>
                        ...
                        const parts = (d.descripcion_libre || '').split('|||');
                        const nombre_manual = parts[0] || '';
                        const detalle_manual = parts.slice(1).join('|||') || '';
                        
                        tr.innerHTML = `
                            <td style="padding-right: 10px;">
                                <input type="hidden" name="detalles[${rowIndex}][tipo]" value="manual">
                                <input type="text" class="form-control" name="detalles[${rowIndex}][nombre_manual]" value="${nombre_manual}" placeholder="Nombre del servicio manual..." required style="font-size: 13px; width: 100%; box-sizing: border-box; font-weight: bold; color: var(--color-primario); background: #fff;">
                            </td>
                            <td style="padding-right: 10px;">
                                <input type="number" class="form-control input-costo" name="detalles[${rowIndex}][precio_unitario]" value="${parseFloat(d.precio_unitario)}" step="0.01" oninput="recalcularTotal()" style="width: 100%; box-sizing: border-box; text-align: right; background: #fff;">
                            </td>
                            <td style="padding-right: 10px;">
                                <input type="text" class="form-control" name="detalles[${rowIndex}][descripcion_libre]" value="${detalle_manual}" placeholder="Detalles adicionales del procedimiento..." style="font-size: 13px; padding: 8px; width: 100%; box-sizing: border-box; background: #fff;">
                            </td>
                            <td style="text-align:center;">
                                <button type="button" class="btn-remove" onclick="this.closest('tr').remove(); revisarFilaVacia(); recalcularTotal();" title="Quitar Servicio" style="border-radius: 50%; width: 24px; height: 24px; padding: 0; line-height: 1;">✖</button>
                            </td>
        ...
        window.onload = function() {
            ...
            if(document.querySelector('input[name="tipo_registro"]:checked').value === 'registrado' && !document.getElementById('IDVehiculo').value) {
                renderizarVehiculos();
            }
        };
    </script>
