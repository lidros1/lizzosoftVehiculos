<?php
/**
 * Lizzosoft Vehículos - Edición de Clientes
 * Ubicación: lizzosoft_vehiculos/CRUD_Clientes/editar_cliente.php
 */
...
if ((!in_array(5, $areas_permitidas) || !in_array(2, $funciones_permitidas)) && !$es_admin) {
    die("<div style='padding:20px; font-family:Arial; color:#721c24; background:#f8d7da;'>Error: No tienes permisos para editar Clientes.</div>");
}
...
try {
    ...
    $mensaje = "Error al inicializar los catálogos del sistema.";
    $tipoMensaje = "error";
}
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (empty($nombre) || empty($apellido)) {
        ...
        try {
            // Verificar duplicidad de documento en la misma empresa (excluyendo al cliente actual) solo si se ingresó uno
            if (!empty($documento)) {
                $stmtCheckDni = $conexion->prepare("SELECT IDCliente FROM clientes WHERE numeroDocumentoCliente = ? AND empresa_id = ? AND sucursal_id = ? AND IDCliente != ?");
                $stmtCheckDni->execute([$documento, $empresa_id, $sucursal_id, $idCliente]);
                if ($stmtCheckDni->fetch()) {
                    throw new Exception("El número de documento ingresado ya pertenece a otro cliente registrado en el sistema.");
                }
            }
            ...
            $sqlUpd = "UPDATE clientes 
                       SET nombre = ?, apellido = ?, IDTipoDocumento = ?, numeroDocumentoCliente = ?, 
                           IDTipoNumeroTelefono = ?, telefono = ?, email = ?, estado = ?
                       WHERE IDCliente = ? AND empresa_id = ?";
            $stmtUpd = $conexion->prepare($sqlUpd);
            $stmtUpd->execute([
                $nombre, $apellido, $idTipoDoc, $documentoFinal, $idTipoTel, $telefonoFinal, $email, $estado, $idCliente, $empresa_id
            ]);
            ...
            $logAccion = "Modificacion de datos de cliente: $apellido, $nombre";
            $stmtLog->execute([$_SESSION['IDUsuario'], $_SESSION['nombreUsuario'], $logAccion, $empresa_id, $sucursal_id]);

            $mensaje = "Los datos del cliente han sido actualizados exitosamente.";
            $tipoMensaje = "exito";
...
try {
    $stmtData = $conexion->prepare("SELECT * FROM clientes WHERE IDCliente = ? AND empresa_id = ? AND sucursal_id = ?");
    $stmtData->execute([$idCliente, $empresa_id, $sucursal_id]);
    $cliente = $stmtData->fetch(PDO::FETCH_ASSOC);

    if (!$cliente) {
        die("<div style='padding:40px; text-align:center; font-family:Arial; color:#721c24;'><h3>Acceso Denegado</h3><p>El cliente solicitado no existe o no pertenece a su organización.</p><a href='listar_clientes.php'>Volver</a></div>");
    }
} catch (PDOException $e) {
    die("Error al consultar los datos del cliente.");
}
?>
<!DOCTYPE html>
<html lang="es">
<head>
    ...
    <title>Editar Cliente - <?php echo htmlspecialchars($config['nombre_empresa']); ?></title>
    <style>
        :root {
        ...
        .form-actions { margin-top: 20px; padding-top: 20px; border-top: 1px solid #eee; display: flex; justify-content: space-between; align-items: center; }
        ...
        <main class="content-area">
            <div class="container">
                <div style="display:flex; justify-content:space-between; align-items:center; margin-bottom:20px;">
                    <h2 class="tarjeta-titulo" style="border:none; margin:0; padding:0;">Editar Cliente</h2>
                ...
                <div class="tarjeta">
                    <div class="form-grid">
                        ...
                        <div class="form-grupo">
                            ...
                            <select name="id_tipo_documento" class="form-control">
                                <?php foreach(($tiposDoc ?? []) as $td): ?>
                                    <option value="<?php echo $td['IDTipoDocumento']; ?>" <?php echo ($cliente['IDTipoDocumento'] == $td['IDTipoDocumento']) ? 'selected' : ''; ?>>
                        ...
                        <div class="form-grupo">
                            ...
                            <input type="text" name="documento" pattern="[0-9]+" title="Solo números" class="form-control" placeholder="Opcional" value="<?php echo htmlspecialchars($cliente['numeroDocumentoCliente'] ?? ''); ?>">
                        </div>

                        <div class="form-grupo">
                            <label>Nombre *</label>
                            <input type="text" name="nombre" class="form-control" required placeholder="..." value="<?php echo htmlspecialchars($cliente['nombre']); ?>">
                        </div>

                        <div class="form-grupo">
                            <label>Apellido *</label>
                            <input type="text" name="apellido" class="form-control" required placeholder="..." value="<?php echo htmlspecialchars($cliente['apellido']); ?>">
                        </div>
                        ...
                        <div class="form-grupo">
                            ...
                            <select name="id_tipo_telefono" class="form-control">
                                <?php foreach(($tiposTel ?? []) as $tt): ?>
                                    <option value="<?php echo $tt['IDTipoNumeroTelefono']; ?>" <?php echo ($cliente['IDTipoNumeroTelefono'] == $tt['IDTipoNumeroTelefono']) ? 'selected' : ''; ?>>
                        ...
                        <div class="form-grupo">
                            ...
                            <input type="tel" name="telefono" pattern="[0-9+\-\s]+" title="Solo números, guiones y espacios" class="form-control" placeholder="..." value="<?php echo htmlspecialchars($cliente['telefono'] > 0 ? $cliente['telefono'] : ''); ?>">
                        </div>

                        <div class="form-grupo full-width">
                            <label>Correo Electrónico</label>
                            <input type="email" name="email" class="form-control" placeholder="..." value="<?php echo htmlspecialchars($cliente['email'] ?? ''); ?>">
                        </div>
